 ...
 * @author Nhatthang
 */
public class SearchFullNameController extends HttpServlet { 
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (searchValue.trim().length() > 0) {
                ...
                List<UserDTO> userList = userDao.searchByFullName(searchValue);
                System.out.println(userList);
                request.setAttribute("userList", userList);
                action = "SearchFullNamePage";
            }
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(SearchFullNameController.class.getName()).log(Level.SEVERE, null, ex);
